...
def addcurrency(request,name,description,currencyname):
    if email:
    ...
    def get(self):
        ...
        if user:
            if user.email() in myemails():
                ...
                content.append("<hr>")
                rows = [[currency.name,currency.description,currency.currencyname,buttonformget("/viewcurrency/" + currency.key.urlsafe(),"View")] for currency in getallcurrencys(self)]
...
class AddCurrency(webapp2.RequestHandler):
    def get(self):
        ...
        if user:
            if user.email() in myemails():
                content.append(html("h1","Add Currency"))
            else:
                content = ['Not Authorized']
                url_linktext = 'Logout'
                content.append(htmllink(url,url_linktext))
